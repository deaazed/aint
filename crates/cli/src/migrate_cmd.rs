...
Widget literals (UI, milestone 44/46): `Role { name: expr ... expr ... }` builds a `Node` value. The widget vocabulary is closed and abstract - never an HTML tag name or CSS property: Page (document root - title/description props, an optional Theme child, one body child), Theme (Light/Dark palette children - accent/background/surface/text/border/on_accent color props), Column/Row (gap/align/wrap), Box (padding/background/corner_radius/border/width/height/grow), Spacer, Responsive (Narrow/Wide children, swapped at a fixed breakpoint), Text/Heading (size/weight/color/level), Button/Link (to, plus the same style props as Box), Image (src/alt/width/height), Input (kind/id), Label (target), List. Each item is either `name: expr` (a prop, must be String, Int, Float, or Bool - size/spacing props are unit-less numbers like `padding: 24`, never a unit string like `"24px"`) or a bare expr (a child - must be Node, String, or List<Node>, spliced in as multiple children); no commas between items. A color-valued prop (`background`/`color`/`border`) is either a literal CSS color or one of the active theme's palette field names. `import ui` provides exactly one renderer, `render(page: Node) -> String`, which only accepts a Page-rooted tree and produces one complete HTML document - there is no separate CSS renderer and no raw-HTML escape hatch. An `infer`/`tool` declaration can return `Node` too. Only migrate a function to widget literals if it is CLEARLY hand-building an HTML or CSS string (nested string_concat/join_lines calls producing tags, or a selector-and-declarations blob) - never introduce widget literals into code that isn't already building markup or a stylesheet, and never invent a role outside the fixed list above.
...
Key stdlib functions: string_concat, string_split, string_replace(s, target, replacement), string_url_decode, string_length/trim/contains/to_upper/to_lower, math_sqrt/pow/floor/ceil/round/abs/min/max, collections_length, time_now_seconds, http_serve(port), render(page).
